...
function Lantier<D>(dbURL: string, opts: InitOptions): Lantier<D> {
	...
	async function request(dbName: keyof D | "_session", path: string, method: string, data: any, adm?: boolean) {
	...
	return {
		use: <N extends keyof D>(dbName: N): Db<D[N]> => {
			const dbObj: Db<D[N]> =  {
				...
				getAll: async keys => {
					const result: any = await dbObj.get(`_all_docs?include_docs=true` + (keys ? `&keys=${JSON.stringify(keys)}` : ""));
